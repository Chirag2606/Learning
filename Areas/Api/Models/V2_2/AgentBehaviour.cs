namespace Cyara.Web.Portal.Areas.Api.Models.V2_2
{
    ...
    {
        public static AgentBehaviour From(CampaignAgentBehaviour cab)
        {
            return new AgentBehaviour
            {
                Agent = AgentSummary.From(cab.Agent),
                Behaviour = BehaviourSummary.From(cab.Behaviour),
                Server = ServerSummary.From(cab.Server)
            };
        }

        public static AgentBehaviour[] ArrayFrom(Shared.Types.Campaign.Campaign campaign)
        {
            if (campaign == null || campaign.Agents == null || campaign.Agents.Count <= 0)
            {
                return null;
            }

            return campaign.Agents.Where(x => x.CampaignId > 0).Select(x => From(x)).ToArray();
        }
