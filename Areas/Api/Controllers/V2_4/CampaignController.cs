namespace Cyara.Web.Portal.Areas.Api.Controllers.V2_4
{
    ...
    using AgentBehaviour = Cyara.Web.Portal.Areas.Api.Models.V2_4.AgentBehaviour;
    using Campaign = Cyara.Web.Portal.Areas.Api.Models.V2_4.Campaign;
    using CampaignRunSummary = Cyara.Web.Portal.Areas.Api.Models.V2.CampaignRunSummary;
    using CampaignTests = Cyara.Web.Portal.Areas.Api.Models.V2_4.CampaignTests;
    using PlanType = Cyara.Domain.Types.Plan.PlanType;
    using RunForUnit = Cyara.Web.Portal.Areas.Api.Models.V2_4.RunForUnit;
    using TestCaseDistributionProfile = Cyara.Web.Portal.Areas.Api.Models.V2_4.TestCaseDistributionProfile;
    using TestCaseProbability = Cyara.Web.Portal.Areas.Api.Models.V2_4.TestCaseProbability;
    ...
    public class CampaignController : BaseApiController
    {
        ...
        public HttpResponseMessage CampaignUpdate(int account, string scope, int id, Campaign campaign)
        {
            ...
            // Check that the UTL parameters are correct
            ...
            // this is an update to existing campaign, convert campaign from database first
        ...
        public HttpResponseMessage RemoveTests(int account, string scope, int id, CampaignTests tests)
        {
            ...
            switch (campaign.Plan.MediaType)
            {
                case MediaType.AgentVoice:
                    ...
                    // Remove the agent behaviors if they are there...
                    if (ccm.Agents != null)
                    {
                        // While DB has Behavior as part of the PK, you can only add an agent once to a campaign.
                        // We only care about agents that are actually included
                        ccm.Agents = ccm.Agents.Where(x => x.CampaignId > 0).ToList();
                        var agentList = ccm.Agents.Join(tests.AgentBehaviourList, i => i.AgentId, o => o.Agent.AgentId, (i, o) => i);
                        ccm.Agents = ccm.Agents.Except(agentList).ToList();
                    }
        ...
        {
            return RunAbortImpl(account, scope, id, AbortType.HardAbort);
        }
        ...
        public HttpResponseMessage RunSchedule(int account, string scope, int id, CampaignRunSummary campaignRunSummary)
        {
            ...
            if (campaignRunSummary == null || campaignRunSummary.Request == null)
            {
            ...
            // If the run date date is sometime before 5 minutes from now, then run it straight away
            ...
            // if this is a load campaign, the campaign service will look after the scheduled run if the account is set up for load authorization.
        ...
        /// <param name="scope">Which area you are using, voice or agent(CCM)</param>
        /// <param name="id">Campaign ID</param>
        /// <param name="abortType">The kind of abort type to be executed</param>
        protected HttpResponseMessage RunAbortImpl(int account, string scope, int id, AbortType abortType)
        {
            // Check that the URL parameters are correct
            var campaignAbortIdentifier = ApiModel.CampaignAbortIdentifier(scope, id, abortType);
        ...
        private HttpResponseMessage CheckPlantypeSupported(Campaign campaign, GenericResponse<Plan> planResponse)
        {
            switch (planResponse.Value.PlanType)
            {
                ...
                case PlanType.Replay:
                    ...
                    {
                        return ApiResponse.Fails(HttpStatusCode.BadRequest, ApiMessages.Content_CampaignInvalidProbability.FormatWith(PlanType.Replay), ApiMessages.Content).Construct(this, Logger);
                    }

                    break;
                case PlanType.Velocity:
                    if (campaign?.Velocity == null)
                    {
                        return ApiResponse.Fails(HttpStatusCode.BadRequest, ApiMessages.Content_MissingElement.FormatWith("Campaign/Velocity"), ApiMessages.Content).Construct(this, Logger);
                    }

                    if (campaign.Velocity.TestCaseDistributionProfile != TestCaseDistributionProfile.RoundRobin)
                    {
                        return ApiResponse.Fails(HttpStatusCode.BadRequest, ApiMessages.Content_CampaignInvalidProbability.FormatWith(PlanType.Velocity), ApiMessages.Content).Construct(this, Logger);
                    }
                ...
                case PlanType.Outbound:
                    ...
                    break;
                case PlanType.Pulse:
                    if (campaign?.Pulse == null)
                    {
                        return ApiResponse.Fails(HttpStatusCode.BadRequest, ApiMessages.Content_MissingElement.FormatWith("Campaign/Pulse"), ApiMessages.Content).Construct(this, Logger);
                    }

                    if (campaign.Pulse.TestCaseDistributionProfile != TestCaseDistributionProfile.RoundRobin)
                    {
                        return ApiResponse.Fails(HttpStatusCode.BadRequest, ApiMessages.Content_CampaignInvalidProbability.FormatWith(PlanType.Pulse), ApiMessages.Content).Construct(this, Logger);
                    }

                    if (campaign.Pulse.Frequency == null)
                    {
                        return ApiResponse.Fails(HttpStatusCode.BadRequest, ApiMessages.Content_InvalidFrequency.FormatWith(PlanType.Pulse), ApiMessages.Content).Construct(this, Logger);
                    }

                    break;
                case PlanType.PulseOutbound:
                    if (campaign?.PulseOutbound == null)
                    {
                        return ApiResponse.Fails(HttpStatusCode.BadRequest, ApiMessages.Content_MissingElement.FormatWith("Campaign/PulseOutbound"), ApiMessages.Content).Construct(this, Logger);
                    }

                    if (campaign.PulseOutbound.TestCaseDistributionProfile != TestCaseDistributionProfile.RoundRobin)
                    {
                        return ApiResponse.Fails(HttpStatusCode.BadRequest, ApiMessages.Content_CampaignInvalidProbability.FormatWith(PlanType.PulseOutbound), ApiMessages.Content).Construct(this, Logger);
                    }

                    if (campaign.PulseOutbound.Frequency == null)
                    {
                        return ApiResponse.Fails(HttpStatusCode.BadRequest, ApiMessages.Content_InvalidFrequency.FormatWith(PlanType.PulseOutbound), ApiMessages.Content).Construct(this, Logger);
                    }

                    if (campaign.PulseOutbound.RunFor == null)
                    {
                        return ApiResponse.Fails(HttpStatusCode.BadRequest, ApiMessages.Content_InvalidRunFor.FormatWith(PlanType.PulseOutbound), ApiMessages.Content).Construct(this, Logger);
                    }
                    else if (campaign.PulseOutbound.RunFor.Unit != RunForUnit.Minutes)
                    {
                        return ApiResponse.Fails(HttpStatusCode.BadRequest, ApiMessages.Content_InvalidRunForUnit.FormatWith(PlanType.PulseOutbound, campaign.PulseOutbound.RunFor.Unit.ToString()), ApiMessages.Content).Construct(this, Logger);
                    }
