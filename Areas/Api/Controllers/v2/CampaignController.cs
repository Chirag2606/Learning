namespace Cyara.Web.Portal.Areas.Api.Controllers.V2
{
    ...
    using Campaign = Cyara.Web.Portal.Areas.Api.Models.V2.Campaign;
    ...
    public class CampaignController : BaseApiController
    {
        ...
        {
            // only voice campaigns can be created
            ...
            // Ensure we have the right plan type, and valid content.
            switch (planResponse.Value.PlanType)
            {
                case PlanType.Replay:
                    ...
                    if (campaign.Replay.TestCaseDistributionProfile != Models.V2.TestCaseDistributionProfile.RoundRobin)
                    {
                    ...
                    {
                        return ApiResponse.Fails(HttpStatusCode.NotAcceptable, ApiMessages.Content_PlanChannelMismatch, ApiMessages.Content_WrongPlan).Construct(this, Logger);
                    }
            ...
            // get the basic model based on plan selected
            var viewModel = ModelBuilder.BuildFromScratch(
                    ...
                    CampaignService,
                    httpContext,
                    false) as VoiceCampaignReplayEditViewModel;

            // map items from API parameters into the viewModel
            campaign.SetOn(viewModel);
            ...
            // as this is a brand new campaign being created, default these ones
            campaignToSave.Frequency = null;
            campaignToSave.Active = true;
            campaignToSave.TotalCallsToMake = 0;
            campaignToSave.RunTime = null;
        ...
        public HttpResponseMessage CampaignUpdate(int account, string scope, int id, Campaign campaign)
        {
            ...
            var viewModel = ModelBuilder.Create(campaignResponse.Value) as VoiceCampaignReplayEditViewModel;
        ...
        public HttpResponseMessage GetTest(int account, string scope, int id)
        {
            ...
            if (loadResponse.IsSuccess && loadResponse.Value != null)
            {
                return ApiResponse.Succeeds(
                    new CampaignTests
                    {
                        AgentBehaviourList = loadResponse.Value.AgentBehaviourList,
                        TestCaseList = loadResponse.Value.TestCaseList
        ...
        public HttpResponseMessage RemoveTests(int account, string scope, int id, CampaignTests tests)
        {
            ...
            switch (campaign.Plan.MediaType)
            {
                case MediaType.AgentVoice:
                    if (tests?.AgentBehaviourList == null || tests.AgentBehaviourList.Count() <= 0)
                    {
                ...
                case MediaType.Voice:
                    if (tests?.TestCaseList == null || tests.TestCaseList.Count() <= 0)
                    {
